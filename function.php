    ...
    $httpGetJson = static function (string $url, int $timeout = 5, int $retries = 1): ?array {
        ...
        do {
            ...
            curl_setopt_array($ch, [
                ...
                CURLOPT_USERAGENT      => 'tronratee/1.2 (+https://example.local)',
                CURLOPT_SSL_VERIFYPEER => true,
                CURLOPT_SSL_VERIFYHOST => 2,
                CURLOPT_IPRESOLVE      => CURL_IPRESOLVE_V4, // بعضی هاست‌ها با IPv6 مشکل دارند
            ...
            if ($errno === 0 && $http >= 200 && $http < 300 && is_string($resp) && $resp !== '') {
                ...
                error_log('Invalid JSON from: ' . $url);
            } else {
                error_log(sprintf(
                    'HTTP GET failed (try %d/%d): %s | http=%d curl_errno=%d',
                    $attempt, $retries + 1, $url, $http, $errno
                ));
            }

            // backoff سبک بین تلاش‌ها
            if ($attempt <= $retries) usleep(150 * 1000);
        } while ($attempt <= $retries);

        return null;
    };

    // ---------- Nobitex ticker (USDTIRT, TRXUSDT, TONUSDT) ----------
    $nobitexTicker = static function (string $symbol) use ($httpGetJson): ?float {
        // گزینه ۱: v2/orderbook (GET)
        $d1 = $httpGetJson('https://api.nobitex.ir/v2/orderbook?symbol=' . urlencode($symbol));
        if (is_array($d1)) {
            if (isset($d1['lastTradePrice']) && is_numeric($d1['lastTradePrice'])) {
                return (float)$d1['lastTradePrice'];
            }
            $ask = $d1['asks'][0][0] ?? null;
            $bid = $d1['bids'][0][0] ?? null;
            if (is_numeric($ask) && is_numeric($bid)) {
                return (float)(($ask + $bid) / 2.0);
            }
        }
        // گزینه ۲: market/stats (GET)
        $d2 = $httpGetJson('https://api.nobitex.ir/market/stats?symbol=' . urlencode($symbol));
        if (is_array($d2)) {
            $node = $d2['stats'][$symbol] ?? ($d2[$symbol] ?? null);
            $latest = is_array($node) ? ($node['latest'] ?? null) : null;
            if (is_numeric($latest)) {
                return (float)$latest;
            }
        }
        error_log('Nobitex ticker failed for ' . $symbol);
        return null;
    };

    // ---------- USD→IRT (به تومان) ----------
    $getUsdToIrr = static function () use ($httpGetJson, $nobitexTicker): ?float {
        // 1) منبع محلی/عملی‌تر: USDTIRT از Nobitex (IRT=تومان ⇒ ×10 برای ریال)
        $usdtIrt = $nobitexTicker('USDTIRT');
        if (is_numeric($usdtIrt) && $usdtIrt > 0) {
            return $usdtIrt;
        }
        // 2) open.er-api.com
        $d2 = $httpGetJson('https://open.er-api.com/v6/latest/USD');
        if (isset($d2['rates']['IRR']) && $d2['rates']['IRR'] > 0) {
            return (float)$d2['rates']['IRR'];
        }
        // 3) exchangerate.host (به عنوان زاپاس سوم)
        $d3 = $httpGetJson('https://api.exchangerate.host/latest?base=USD&symbols=IRR');
        if (isset($d3['rates']['IRR']) && $d3['rates']['IRR'] > 0) {
            return (float)$d3['rates']['IRR'];
        }

        error_log('Failed to fetch USD→IRR from all sources');
        return null;
    };

    // ---------- صرافی‌های بین‌المللی ----------
    $getPriceFromBinance = static function (string $symbolUsdt) use ($httpGetJson): ?float {
        $d = $httpGetJson('https://api.binance.com/api/v3/ticker/price?symbol=' . urlencode($symbolUsdt));
        return (isset($d['price']) && is_numeric($d['price'])) ? (float)$d['price'] : null;
    };
    $getPriceFromKuCoin = static function (string $symbolDash) use ($httpGetJson): ?float {
        $d = $httpGetJson('https://api.kucoin.com/api/v1/market/orderbook/level1?symbol=' . urlencode($symbolDash));
        if (!is_array($d) || (isset($d['code']) && (string)$d['code'] !== '200000')) return null;
        return (isset($d['data']['price']) && is_numeric($d['data']['price'])) ? (float)$d['data']['price'] : null;
    };

    // ---------- قیمت TRX/TON به USDT ----------
    $getAssetUsdt = static function (string $asset) use ($getPriceFromBinance, $getPriceFromKuCoin, $nobitexTicker): ?float {
        $map = [
            'TRX' => ['binance' => 'TRXUSDT', 'kucoin' => 'TRX-USDT', 'nobitex' => 'TRXUSDT'],
            'TON' => ['binance' => 'TONUSDT', 'kucoin' => 'TON-USDT', 'nobitex' => 'TONUSDT'],
        ];
        if (!isset($map[$asset])) return null;

        $p = $getPriceFromBinance($map[$asset]['binance']);
        if (is_numeric($p) && $p > 0) return $p;

        $p = $getPriceFromKuCoin($map[$asset]['kucoin']);
        if (is_numeric($p) && $p > 0) return $p;

        $p = $nobitexTicker($map[$asset]['nobitex']);
        if (is_numeric($p) && $p > 0) return $p;

        error_log("Failed to fetch $asset/USDT from Binance/KuCoin/Nobitex");
        return null;
    };

    // ---------- Fetch ----------
    $usdToIrr = $getUsdToIrr();  // ریال بر هر دلار/تتر
    if ($usdToIrr === null) {
        return ['ok' => false, 'result' => []];
    }

    $trxUsdt = $getAssetUsdt('TRX');
    $tonUsdt = $getAssetUsdt('TON');
    if ($trxUsdt === null || $tonUsdt === null) {
        return ['ok' => false, 'result' => []];
    }

    // ---------- Helpers ----------
    $toToman = static function (float $tomanValue): float { return round($tomanValue, 2); };

    // USDT≈USD ⇒ تتر/ریال = usdToIrr
    $result = [
        'TRX' => $toToman($trxUsdt * $usdToIrr),
        'Ton' => $toToman($tonUsdt * $usdToIrr),
        'USD' => $toToman($usdToIrr),
    ];
