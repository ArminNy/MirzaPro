...
try {
    $pdo->exec("CREATE TABLE IF NOT EXISTS `x_ui` (
        ...
        `codepanel` VARCHAR(128) NOT NULL UNIQUE,
        `setting` LONGTEXT NULL,
        `updated_at` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        PRIMARY KEY (`id`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;");
} catch (Throwable $e) {
    // ignore creation error to not break page; will fail later if query runs
}
...
if (isset($_GET['action']) && $_GET['action'] == "save"){
    ...
    $existingByName = select("product","*","name_product",$name_product,"select");
    if($existingByName && isset($existingByName['id']) && (string)$existingByName['id'] !== (string)$id_product){
        $statusmessage = true;
        $infomesssage ="نام محصول وجود دارد.";
    }else{
        if((int)$dupCount == 0 && $product['name_product'] != $name_product){
    ...
    if($changesCount > 0){
        $statusmessage = true;
        $infomesssage = 'تنظیمات ذخیره شد';
        $product = select("product","*","id",$id_product,"select");
    } else {
        $statusmessage = true;
        $infomesssage = 'ذخیره ناموفق - تغییری در تنظیمات داده نشده است';
    }
}
}
}
...
?>
    ...
    if (str_contains($infomesssage, 'ناموفق')) {
        $alertClass = 'alert-warning';
    } elseif (str_contains($infomesssage, 'وجود دارد')) {
        $alertClass = 'alert-danger';
    }
    echo "<div class='alert $alertClass' role='alert' style='margin-top:10px;'>$infomesssage</div>";
} ?>
                <div class="row">
